...
static void channel_serveNoidRequestSelf(void *vself, Noid *oid, void *vserver, int command){
	...
	switch(command){
		case CMD_NOID_GET_FTS:{
				Fts fts;
				fts.value = channel_getOutput(self);
				fts.tm = getCurrentTs();
				fts.success = YES;
		...
		case CMD_NOID_GET_STATE:
			noidServer_sendI(oid, server, channel_getState(self));
			return;
		case CMD_NOID_GET_ERROR:
			noidServer_sendI(oid, server, channel_getError(self));
			return;
		case CMD_NOID_START:
			channel_start(self);
			acpls_reset(server);
		...
		case CMD_NOID_GET_DEVICE_KIND:
			noidServer_sendI(oid, server, DEVICE_KIND_TIMER);
			return;
