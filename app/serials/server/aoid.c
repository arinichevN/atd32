...
static int aoidServer_supportedGnS(int command){
	switch(command){
		case CMD_AOID_GET_NVRAM_VALUE: case CMD_AOID_SET_NVRAM_VALUE: 
			return 1;
	}
	return 0;
}
...
void aoidServer_sendSupportedGnS(Aoid *oid, Acpls *server){
	aoidServer_sendSupportedCommand(oid, server, aoidServer_supportedGnS);
}
