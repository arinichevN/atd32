...
void acnf_getRTCDate(ACPLS *item, HardwareSerial *serial){
	if(!srvc_forThisApp(item)) {ACPLS_RESET return;}
	unsigned long out = dt_dtToIDate(rtc.now());
	int r = acp_buildPackIUlI(item->acpl->buf, ACP_BUF_MAX_LENGTH, ACP_SIGN_RESPONSE, app.id, out, 1);
	if(r){ACPLS_PREP_SEND}else{ACPLS_RESET}
}

void acnf_getRTCTime(ACPLS *item, HardwareSerial *serial){
	if(!srvc_forThisApp(item)) {ACPLS_RESET return;}
